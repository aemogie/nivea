...
let
  inherit (config.paint.active.palette)
    base
    text
    surface2
    subtext0
    ...
    extraConfig =
      let
        colors = builtins.mapAttrs (_: c: "${toString c.r},${toString c.g},${toString c.b}") {
          palette-black = if isDark then surface1 else surface2;
          palette-red = red;
          palette-green = green;
          ...
          palette-cyan = teal;
          palette-light-grey = if isDark then subtext1 else subtext0;

          palette-dark-grey = if isDark then surface2 else surface1;
          ...
          palette-white = if isDark then subtext0 else subtext1;
