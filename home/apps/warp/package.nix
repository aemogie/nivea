...
let
  versions = {
    stable = "0.2024.02.20.08.01.stable_01";
  ...
  strTitleCase =
    str:
    with builtins;
    "${lib.strings.toUpper (substring 0 1 str)}${substring 1 (stringLength str) str}";
  mkSrc =
    channel: platform: sha256:
    fetchurl {
      inherit sha256;
      url = "https://releases.warp.dev/${channel}/v${versions."${channel}"}/Warp${
        if channel == "stable" then "" else strTitleCase channel
      }${platformSuffixes."${platform}"}";
    };
  sources = {
    linux = {
      stable = mkSrc "stable" "linux" "sha256-541IHjrtUGzZwQh5+q4M27/UF2ZTqhznPX6ieh2nqUQ=";
  ...
  if stdenvNoCC.isLinux then
    {
      ...
      installPhase = ''
        ...
        substituteInPlace $out/share/applications/dev.warp.Warp*.desktop \
          --replace-warn 'Exec=${channel} %U' 'Exec=warp-term %U'
        install -m 444 -D usr/share/icons/hicolor/512x512/apps/dev.warp.Warp*.png \
          -t $out/share/icons/hicolor/512x512/apps

        install -m 755 -D usr/bin/warp $out/bin/warp-term${
          if channel == "stable" then "" else "-${channel}"
        }
