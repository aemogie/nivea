...
in
{
  ...
  programs.foot = lib.mkIf autostart { settings.main.shell = config.programs.zellij.baseCommand; };
  programs.zellij = {
    configs.default =
      #kdl
      ''
        ...
        editor =
          ...
          ''
            layout {
              ...
              tab name="editor" {
                pane name="editor" borderless=true command="$SHELL" { args "-l" "-c" "$env.config.hooks.pre_prompt | first | do $in; ^$env.EDITOR ."; }
